...
| # | 改善点 | 根拠 | 優先 |
|---|--------|------|:---:|
| 1 | ~~提案プロセス（`processes/proposed.json` + `prompts/proposed/*.md`）が未実装。検証の核である「コードレビュー省略率」が測定不能~~ → **完了**（4フェーズ構成で実装。②でテストコードを作成しレビュー、③が `review-scope-<STORY>.json` を出力し `reviews.jsonl` の `scope_*` 列へ記録。設計は [proposed-process-plan.md](proposed-process-plan.md)） | `harness/processes/proposed.json` | ~~**P0**~~ 完了 |
...
- **最終フェーズのゲートも計測対象にした。** `existing` の `e2e-run` と `baseline` の `build` は
  ...
  **比較対象群のレビュー時間が丸ごと落ちて主指標（−30%）が成立しない**。
  そこで**最終フェーズ完了時も `awaiting-review` で止め**、`-Continue` で最終ラウンドを閉じたときに
  初めて `done` にする（次フェーズへは進まない）。当初は最終フェーズ完了時点で `done` にしつつ
  `done` でも `-Continue` を受け付ける形にしていたが、それだと `-Revise` が status チェックで弾かれ
  **最終ゲートの指摘だけ反映できなかった**ため、他のゲートと同じ状態遷移に揃えた
  （`done` かつラウンド未クローズの旧 state は `-Revise` / `-Continue` 側で救済する）。
